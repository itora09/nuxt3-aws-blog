import { Category } from '~/server/models/dynamoDB/Category'
import {
  HTTP_ERROR_CODES,
  HTTP_SUCCESS_CODES,
...
import { execute } from '~/server/services/CategoryService'

export default defineEventHandler(async (event) => {
  const body = await readBody(event)
  const data: Category = body.data
  ...
  try {
    ...
    if (error instanceof CommonError) {
      throw createError({
        ...
        message: error.message || 'Category execute failed.',
        stack: '',
      })
    }
    throw createError({
